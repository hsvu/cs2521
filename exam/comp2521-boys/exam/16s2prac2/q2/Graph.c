...
int within(Graph g, Vertex s, int d, Vertex *vs)
{
  ...
  while (!QueueIsEmpty(new)){
    int curr = QueueLeave(new);
    visited[curr] = 1;

    if (distance[curr] > d) break;
    counter++;

    // add all neighbours of curr to queue
    for (int b = 0; b < g->nV; b++){
      if (!visited[g->edges[curr][b]]) QueueJoin(new, g->edges[curr][b]);
    }
    showQueue(new);
