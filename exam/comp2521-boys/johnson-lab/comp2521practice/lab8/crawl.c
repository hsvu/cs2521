...
int main(int argc, char **argv)
{
    ...
    while(!emptyQueue(queue)&&nVertices(graph)!=maxURLs){
    
        current = leaveQueue(queue);
	    if (!(handle = url_fopen(current, "r"))) {
	    ...
	    while(!url_feof(handle)) {
		   ...
		   // fputs(buffer,stdout);
		    ...
		    while ((pos = GetNextURL(buffer, current, result, pos)) > 0) {
			    
			if(isConnected(graph, current,result)==0){
		        
		        addEdge(graph,current,result);
		    }
		    if(isElem(seen,result)==0){
		        printf("Found: '%s'\n",result);
		        enterQueue(queue,result);
