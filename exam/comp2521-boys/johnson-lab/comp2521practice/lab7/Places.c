...
int binary(char*name ,int start,int end){
    if(end<start){
        return NOWHERE;
    }
    int mid = (start+end)/2;
    ...
    if(strcmp(places[mid].name,name)<0){
        return binary(name,mid+1,end);
    }else{
        return binary(name,start,mid-1);
    }
