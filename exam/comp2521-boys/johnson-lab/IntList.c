...
{
    //checks whether they are in order and returns non-zero
    struct IntListNode* insert=newIntListNode(v);
    ...
    // handles case for no number in the list
    if(L->size==0){
        L->size=1;
        L->first=insert;
        L->last=insert;
    ...
    // checks the first number  in the list
    if(L->first->data>=v){
        insert->next=L->first;
        L->first=insert;
        return;
    }
    struct IntListNode* iterator=L->first;
